...
void restoreBackup(){
	char const *path_bak = "config.bak";						// Names of the files
	char const *path_bin = "config.bin";

	if(!SD.exists(path_bak)){									// If there is no backup file
		return;													// Return, don't try to restore it
	}

	if(SD.exists(path_bin)){									// If there is a bin file
		SD.remove(path_bin);									// Remove it
	}

	File backupFile = SD.open(path_bak, O_WRONLY | O_CREAT);	// Open the backup file
	backupFile.rename(SD.vwd(), path_bin);						// Restore it!
